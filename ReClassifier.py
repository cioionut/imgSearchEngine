...
class S3Downloader(object):
    ...
    def __init__(self):
        ...
        self.downloader = tr.S3Transfer(boto3.client('s3'))
    ...
    def get_caffe_files(self):
        ...
        for caffe_file in caffe_files:
            ...
            self.downloader.download_file(self.BUCKET_NAME, self.CAFFE_FILES_FOLDER + caffe_file[0], caffe_file[1])
    ...
    def get_images(self, images_to_download):
        ...
        for image in images_to_download:
            ...
            self.downloader.download_file(self.BUCKET_NAME, self.IMAGES_FOLDER + image, image)
...
if __name__ == '__main__':
    logging.basicConfig(filename='ReClassifier.log', level=logging.INFO,
                        format='%(asctime)s - %(levelname)s: %(message)s.')
    logging.info('ReClassifier started')

    try:
        downloader = S3Downloader()

        if should_download_caffe_files():
            downloader.get_caffe_files()
        else:
            logging.info("'-nodownload' option was specified. Using old files")

        classifier = ImagenetClassifier(**ImagenetClassifier.default_args)
        receiver = SQSMessageReceiver()
        communicator = DynamoCommunicator()
        contents = receiver.get_messages_contents()

        while contents:
            image_names = []

            for content in contents:
                image_names = image_names + content.split()

            downloader.get_images(image_names)
            opened_images = [caffe.io.load_image(img) for img in image_names]
            results = classifier.classify_images(opened_images)

            if not results[0]:
                raise Exception('Classification error. ' + results[1])

            classes = communicator.get_classes(image_names)
            index = 0

            for pair in zip(image_names, results[1]):
                logging.info('Classes for image ' + pair[0] + ': ' + str(pair[1]))
                new_classes_set = set(pair[1])
                old_classes_set = set(classes.get(pair[0], []))

                classes_item_inserted = communicator.add_image_to_classes(pair[0], new_classes_set - old_classes_set)
                classes_item_deleted = communicator.remove_image_from_classes(pair[0],
                                                                              old_classes_set - new_classes_set)

                if classes_item_deleted or classes_item_inserted:
                    results[1][index] = list(old_classes_set - classes_item_deleted | classes_item_inserted)
                else:
                    results[1][index] = []
            ...
            communicator.update_images_table(image_names, results[1])
            contents = receiver.get_messages_contents()

    except Exception as err:
        if len(err.message):
            logging.error('Caught exception with message ' + err.message)
        else:
            logging.error('Caught exception without message.')

    finally:
        logging.info('ReClassifier stopping')
